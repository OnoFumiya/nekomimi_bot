...
class LocationSetting(Node):
    def __init__(self):
        ...
        self.tk = tk.Tk()
        self.iconfile = tk.PhotoImage(file=os.path.join(get_package_share_directory('nekomimi_bot_navigation'), 'img', 'mapping.png'))
    ...
    def callback_nav_goal(self, msg):
        ...
        iconfile = tk.PhotoImage(file=os.path.join(get_package_share_directory('nekomimi_bot_navigation'), 'img', 'mapping.png'))
    ...
    def select_location_file(self):
        proc = Popen(["zenity", "--file-selection", "--save", "--confirm-overwrite", "--filename=/home/" + str(os.getenv("USERNAME")) + "/colcon_ws/src/nekomimi_bot/nekomimi_bot_navigation/location/location_example.yaml"],
            stdout=PIPE,
            shell=False)
